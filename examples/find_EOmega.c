...
int main(int argc, char **argv) {
	double g = 0.5;
        ...
        double gis[6] = { 0.0, 0.5, 1.0, 1.5, 2.0, 3.0 };
        for (uint i = 0; i < sizeof(gis)/sizeof(*gis); ++i) {
                char fname_ener[256];
                sprintf(fname_ener, "./results/data/energies/energies_n%u_%.1lfg.txt", n_part, gis[i]);
                FILE* file_ener = rgc_fopen(fname_ener, "a");

                EigenSolutionSet sol = fock_eigen_vt_solve(gis[i], set.size, T, V);
                fprintf(file_ener, "%u", tot_momentum);
                for (uint i = 0; i < 8; ++i) {
                        fprintf(file_ener, " %lf", sol.values[i]);
                }
                fputc('\n', file_ener);

                rgc_fclose(file_ener);
                fock_eigen_free(&sol);
        }
        return 0;

        // Diagonalize G function print the last 5 elements (Biggest lambdas)
        EigenSolutionSet sol = fock_eigen_vt_solve(g, set.size, T, V);
        G1State gfunc = fock_g1_alloc(set, sol.vectors);

        // Print G function
        uint ixy = 0;
        double rho_arr[100*100];
        for (uint iy = 0; iy < 100; ++iy) {
                for (uint ix = 0; ix < 100; ++ix) {
                        double y = -1.0 + iy*(2.0/99);
                        double x = -1.0 + ix*(2.0/99);
                        rho_arr[ixy] = cabs(fock_g1_compute(&gfunc, x, y, x, y));
                        ++ixy;
                }
        }
        char fname_rho[256];
        sprintf(fname_rho, "./results/data/p6m%u/rho_g%.1lf.dat", tot_momentum, g);
        util_bin_tofile(rho_arr, 100*100*sizeof(*rho_arr), fname_rho);
